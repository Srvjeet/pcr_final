...
db.sequelize
  ...
  .catch((e) => {
    ...
    fs.appendFile(
      "../logs/log_rest.log",
      `db init error ${e.toString()}, ${JSON.stringify(
        e
...
export async function seedDB(db: any) {
  if (
    !(
      process.env.INIT_MANAGER_USERNAME &&
  ...
  if (isDBSeed) {
    ...
    if ((await db.models.managers.findOne()) == null) {
      await db.models.managers.create({
        ...
        pwhash: await bcrypt.hash(process.env.INIT_MANAGER_PW as string, 10),
...
process.on("exit", (code) => {
  if (code) {
    if (process.env.ENV_TEST) {
      ...
      fs.appendFile(
        "../logs/log_rest.log",
        `uncaught exception. stack: ${code} ${new Date().toISOString()} \n`,
        console.warn
      );
    }
    process.exit(1);
